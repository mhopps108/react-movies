...
const useTmdbUrl = () => {
  ...
  useEffect(() => {
    ...
    setStartDate("dates" in data ? data.dates.minimum : 0);
    setEndDate("dates" in data ? data.dates.maximum : 0);
  }, [data]);
  ...
  return [{ data, isLoading, isError, list, setList, setReleaseType, setPage }];
};
